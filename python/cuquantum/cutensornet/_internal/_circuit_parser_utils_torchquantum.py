...
def remove_measurements(circuit):
    ...
    # NOTE: if I'm understanding correct, there is no real measurement for
    #   tq with the circuit (it's all separate)
...
def get_inverse_circuit(circuit):
    ...
    """
    # TODO: write a new tq method to invert the circuit
...
def is_primitive_gate(operation):
    ...
    """
    # for now, unclear if we support non-primitives 
    #   (i guess layers but that'll be a later problem)
    return True
...
def get_decomposed_gates(circuit, qubit_map=None, gates=None, gate_process_func=None, global_phase=0):
    ...
    """
    # I *think* this should be enough, but we may need to loop through 
    #   for things like gates
    return circuit.op_history
    # if gates is None:
    #     gates = []
